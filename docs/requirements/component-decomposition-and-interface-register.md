...
| <a id="if-veh-007"></a>`IF-VEH-007` | `CMP-VISS` | `CMP-RUNTIME` | Distinct purpose-bound per-Unit `PLATFORM_UPDATE_RUNTIME` mTLS role with one connection and only ten read-only Safe Stop paths: monotonic `FrameId`, applied control mode, transition/reset generation and discontinuity, factual speed, throttle and brake with freshness semantics | Gateway is physical-state authority; OEM Component Runtime evaluates the accepted Safe Stop policy | `NEW`; VISS 1.1.0 and Safe Stop 1.1.0 contracts accepted, implementation open |
