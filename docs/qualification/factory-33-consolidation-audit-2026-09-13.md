...
The following table is the **pre-publication snapshot**. The subsequent user-
authorized [publication and branch cleanup](factory-33-source-checkpoint-2026-09-13.md)
committed and pushed the working source to all seven main branches, with
return-point tags. It supersedes the publication blockers in this historical
table; it does not change the image's original build-source SHA.
...
| OPEN-03 — Upstream review; source publication closed | All seven source repositories are published on main with checkpoint tags; temporary branches were removed after preserving unique historical tips. Native SM/CM fixes are integrated in .33 but upstream acceptance is not established. | [Published source and cleanup receipt](factory-33-source-checkpoint-2026-09-13.md); preserve exact patches and compact tests, with no runtime changes. | Submit/reconcile AosCore review separately. No images, binaries, credentials or private Cloud source enter Git. |
