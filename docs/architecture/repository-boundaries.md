...
| `brake-health-cloud` | Function Team 1 backend, Brake Health Function Dashboard and client integration with the common publication helper surface pre-bound to `brake-sp1` | Function Team 1 Cloud product; isolated source foundation `68fe61b` exists over governance baseline `6da2926`; preserved Data candidate `a4b5b33` has an authorized bounded source correction but is not accepted/integrated until it passes; its current local PKCS#12 remains outside the repository, browser and container |
