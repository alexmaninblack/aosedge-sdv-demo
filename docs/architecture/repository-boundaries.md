...
| `brake-health-cloud` | Function Team 1 backend, Brake Health Function Dashboard and client integration with the common publication helper surface pre-bound to `brake-sp1` | Function Team 1 Cloud product; governance-only baseline `6da2926` exists; product implementation is `NEW`; its current local PKCS#12 remains outside the repository, browser and container |
