...
"""Tests for the R6.1 signed-provider verifier."""
...
SCRIPT = ROOT / "scripts" / "verify-r6-1-signed-provider"


def load_validator():
    loader = importlib.machinery.SourceFileLoader("r6_1_signed_provider", str(SCRIPT))
    spec = importlib.util.spec_from_loader(loader.name, loader)
    if spec is None:
        raise RuntimeError("cannot create signed-provider validator module spec")
...
class R61SignedProviderTests(unittest.TestCase):
    def setUp(self) -> None:
        self.temporary = tempfile.TemporaryDirectory()
        self.root = Path(self.temporary.name)
        self.candidate = self.root / "candidate"
        self.candidate.mkdir()
        self.layer_name = "vehicle-data-provider/provider-0.2.0-arm64.tar"
        self.layer = b"accepted-provider-layer"
        self.config = b'{"schemaVersion":2}\n'
        layer_path = self.candidate / self.layer_name
        layer_path.parent.mkdir()
        layer_path.write_bytes(self.layer)
        (self.candidate / "config.yaml").write_bytes(self.config)
        (self.candidate / "candidate.json").write_text(
            json.dumps(
                {
                    "config": {"sha256": hashlib.sha256(self.config).hexdigest()},
                    "layer": {
                        "fileName": self.layer_name,
                        "sha256": hashlib.sha256(self.layer).hexdigest(),
                        "size": len(self.layer),
                    },
        ...
        self.write_bundle(self.layer)

    def tearDown(self) -> None:
        self.temporary.cleanup()

    def write_bundle(self, layer: bytes, corrupt_signature: bool = False) -> None:
        inner = archive({"config.yaml": self.config, self.layer_name: layer})
        payload = {
            "data": [
                {
                    "name": "batch.tar.gz",
    ...
    def test_valid_signed_bundle_passes(self) -> None:
        ...
        self.assertEqual("0.2.0", evidence["version"])
        self.assertEqual(hashlib.sha256(self.layer).hexdigest(), evidence["layerSha256"])

    def test_changed_embedded_layer_is_rejected(self) -> None:
        self.write_bundle(b"changed-provider-layer")
    ...
    def test_invalid_rs256_signature_is_rejected(self) -> None:
        self.write_bundle(self.layer, corrupt_signature=True)
