...
class R61DisposableVMTests(unittest.TestCase):
    def test_base_is_read_only_hashed_and_variant_scoped(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            try:
                ...
                rejected_bootstrap_image = (
                    root
                    / "bootstrap-6.1.1-maninblack.3"
                    / "main-qemuarm64.img"
                )
                rejected_bootstrap_image.parent.mkdir()
                rejected_bootstrap_image.write_bytes(b"r61-rejected-bootstrap-image")
                rejected_bootstrap_image.chmod(0o444)
                with self.assertRaisesRegex(
                    HELPER.QualificationError, "bootstrap-6.1.1-maninblack.11"
                ):
                    HELPER.create_layout(
                        "bootstrap",
                        str(rejected_bootstrap_image),
                        HELPER.sha256(rejected_bootstrap_image),
                    )
    ...
    def test_guest_gate_separates_upstream_and_empty_project_store(self) -> None:
        ...
        self.assertNotIn("store-side-load", guest_gate)
    ...
    def test_store_fixture_is_bounded_persistent_and_fail_closed(self) -> None:
        ...
        self.assertIn('content_mode=${2:-empty}', fixture)
        self.assertNotIn('security_mode=', fixture)
        self.assertNotIn('store-side-load', fixture)
    ...
    def test_provider_integration_runs_the_complete_enforcing_matrix(self) -> None:
        integration = (
            ROOT / "scripts" / "guest" / "r6-1-provider-integration-check"
        ).read_text(encoding="utf-8")
        self.assertIn("usage: r6-1-provider-integration-check", integration)
        self.assertIn("generic ARM64 Service Manager runtime matrix failed", integration)
        self.assertIn("real provider update and rollback runtime gate failed", integration)
        self.assertIn("provider qualification produced an SELinux AVC denial", integration)
        self.assertNotIn("policy-discovery", integration)
        self.assertNotIn("store-delta", integration)
