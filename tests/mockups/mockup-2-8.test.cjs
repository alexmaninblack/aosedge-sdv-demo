...
const source = fs.readFileSync(path.join(mockupPath, 'aosedge-demo-interaction-mockup-2-8.source.html'), 'utf8');
...
test('Standalone contains the same reviewed state machine',()=>{const html=fs.readFileSync(path.join(mockupPath,'aosedge-demo-interaction-mockup-2-8.html'),'utf8');const section=s=>s.slice(s.indexOf('  const names ='),s.lastIndexOf('})();')+5);assert.equal(section(html),section(source));assert.match(html,/CPU · DMIPS/);assert.doesNotMatch(section(html),/data-cpu-bar/);});
...
test('Offline local VDP apply leaves Cloud on its last report until reconnect',()=>{
 ...
 assert.equal(e.value('state.cloud.online'),false);assert.equal(e.value("Boolean(state.completed['vdp:17.0.0'])"),false);
 e=e.reload();e.run('observe()');assert.equal(e.value('state.cloud.unit.vdp.version'),'16.0.0');
 e.run('toggleNetwork()');e.advance(1000);assert.equal(e.value('state.cloud.unit.vdp.version'),'17.0.0');assert.equal(e.value('state.cloud.pending.vdp'),null);
...
test('Disconnect grace period cannot leak local inventory or service runtime to Cloud',()=>{
 ...
 e.advance(1000);assert.equal(e.value('state.cloud.online'),false);assert.equal(e.value('state.cloud.runtime.brake'),'Running');
});
...
test('Browser: actual buttons, full VDP/Brake/Tire story, offline, Park and Retire', {skip:!process.env.MOCKUP_PLAYWRIGHT,timeout:90000}, async()=>{
  ...
  try {
    ...
    const html=fs.readFileSync(path.join(mockupPath,'aosedge-demo-interaction-mockup-2-8.html'),'utf8');
    ...
    const read=()=>page.evaluate(()=>JSON.parse(localStorage.getItem('aosedge-studio-mockup-2.8')).state);
    ...
    for(const size of [{width:1728,height:1117},{width:1512,height:982},{width:1280,height:720}]){
      ...
      assert.ok(fit.width<=size.width&&fit.height<=size.height,'No page scrolling');
      assert.equal(await page.locator('.dw-slot button:not([hidden]) .b2-icon').count(),3);
    }
    await page.setViewportSize({width:1728,height:1117});await tick(100);
    if(process.env.MOCKUP_SCREENSHOT)await page.screenshot({path:process.env.MOCKUP_SCREENSHOT});
    await click('[data-page="cloud"]');assert.match(await page.locator('[data-cpu]').innerText(),/DMIPS/);
...
test('Browser: offline Cloud display and Safe Stop loss during activation', {skip:!process.env.MOCKUP_PLAYWRIGHT,timeout:60000}, async()=>{
 ...
 try{for(const scenario of ['offline','gate']){
   ...
   await page.addInitScript(saved=>localStorage.setItem('aosedge-studio-mockup-2.8',saved),e.saved());
   await page.clock.install();await page.goto('http://studio-mock.invalid/');
