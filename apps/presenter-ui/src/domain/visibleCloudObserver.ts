...
export class VisibleCloudObserver {
  ...
  refresh = (): Promise<void> => {
    ...
    this.slowTimer = setTimeout(() => {
      if (this.state.loading && this.state.observation?.state === "CURRENT") {
        this.state = { ...this.state, observation: { ...this.state.observation, state: "STALE", reason: "CLOUD_READ_SLOW" } };
        this.emit();
      }
    }, 15000);
    this.flight = Promise.resolve().then(this.read).catch(unavailable).then((incoming) => {
      ...
      const observation = incoming.state !== "CURRENT" && old?.value && sameBinding
        ? { ...old, publication: incoming.publication ?? old.publication, publications: incoming.publications ?? old.publications, serviceReleases: incoming.serviceReleases ?? old.serviceReleases, serviceReleasesState: incoming.serviceReleasesState ?? old.serviceReleasesState, state: "STALE" as const, reason: incoming.reason ?? "AOS_CLOUD_STATE_UNAVAILABLE" }
        : incoming;
