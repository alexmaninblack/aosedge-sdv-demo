...
export function BackendEvidence({ team, unitSystemUid, expectedVersion, binding, onEvidence, retiring = false, observation }: { team: Team; unitSystemUid?: string; expectedVersion?: string; binding?: BackendBinding; onEvidence?: (version: string) => void; retiring?: boolean; observation?: BackendModel }) {
  ...
  return <section className={`studio-cloud${observation ? " is-dialog-content" : ""}`} aria-label={`${team} backend evidence`}>
    ...
    <div className="studio-result-card"><div><small>{mockMode ? "Latest mock result" : "Latest product result"}{(mockMode ? !available && mock : summary.lastKnown) ? " · last known" : ""}</small><h3>{latest ? mockMode ? readable(assessmentContent.currentBand ?? assessmentContent.condition ?? content.status ?? latest.message.messageType) : summary.title : summary.resetState === "CLEARED" ? "No new result after reset" : summary.resetState === "UNCERTAIN" ? "Reset outcome unconfirmed" : "No result yet"}</h3>
      {!mockMode && !latest && <>{expectedVersion && <small>{summary.resetState === "CLEARED" ? `No new result for release ${expectedVersion} after the confirmed reset.` : `No result for release ${expectedVersion} yet.`}</small>}<p>{summary.guidance}</p>{summary.continuityNote && <p>{summary.continuityNote}</p>}{Boolean(mock?.records.length) && <small>{mock?.records.some(row => isProductResult(row, team)) ? "Earlier results remain in Records." : "Service records remain in Records."}</small>}</>}
